...
def process_data_in_batches(dataset, batch_size=100):
    for start_idx in range(0, len(dataset), batch_size):
        ...
        for i in range(start_idx, end_idx):
            if dataset.dataset[0][0].shape[-1] != dataset.dataset[0][0].shape[-2]:
                ...
                batch_data.append(crop(torch.Tensor(dataset.dataset[i][0])).numpy())
            else:
                batch_data.append(dataset.dataset[i][0])
        yield np.array(batch_data), dataset.indices[start_idx:end_idx]
    ...
    def __init__(self, dataset, trigger_label=0, mode='train', epsilon=0.1, pos='top-left', attack_type='static', time_step=16,
        ...
        if type(dataset) == torch.utils.data.Subset:
            ...
            if os.path.exists(path_targets) and os.path.exists(path_data):
                ...
                for batch_data, batch_indices in process_data_in_batches(dataset):
                    data.append(torch.Tensor(batch_data))
...
def create_backdoor_data_loader(args):
    ...
    test_data_ori_loader = DataLoader(
        dataset=test_data_ori, batch_size=args.batch_size, shuffle=False, num_workers=0)
    test_data_tri_loader = DataLoader(
        dataset=test_data_tri, batch_size=args.batch_size, shuffle=False, num_workers=0)
...
def hash_frame(frame):
    ...
    hash_data = bytearray()
    while len(hash_data) < frame.size:  
